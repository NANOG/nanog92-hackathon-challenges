...
GQL_ENDPOINT = "https://n92-nautobot.hackathon.nanog.org/api/graphql/"
CSRF_TOKEN_ENDPOINT = "https://n92-nautobot.hackathon.nanog.org/api/"
    ...
    data = await g.fetch_data(
        ...
        query ($name: [String]) {
            devices(name:$name){
                id
                name
                interfaces(tags: "interface-fabric") {
                    connected_interface {
                        ip_addresses {
                            ip_version
                            address
